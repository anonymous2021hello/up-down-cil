...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, adjust_type=None):
        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size)

        outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        outputs_adjust = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        if self.opt.sup_nde:
            outputs_nde = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        ...
        for i in range(seq.size(1) - 1):
            ...
            if self.opt.cexe  and adjust_type != None:
                # pdb.set_trace()
                #Here, we group the adjust tyep for batch processing. 
                output = fc_feats.new_zeros(batch_size, self.vocab_size+1)
                output_adjust = fc_feats.new_zeros(batch_size, self.vocab_size+1)
                if self.opt.sup_nde:
                    output_nde = fc_feats.new_zeros(batch_size, self.vocab_size+1)
                tmp_state = (state[0].clone(), state[1].clone())
                if (adjust_type[:,i] == 0).sum() !=0:
                    select_mask0 = (adjust_type[:,i] == 0)
                    if self.opt.sup_nde:
                        output0, state0, logprob_interven0, logprob_nde = self.get_logprobs_state(it[select_mask0], p_fc_feats[select_mask0], p_att_feats[select_mask0], pp_att_feats[select_mask0], p_att_masks, (state[0][:,select_mask0,:],state[1][:,select_mask0,:]))
                    else:
                        output0, state0, logprob_interven0 = self.get_logprobs_state(it[select_mask0], p_fc_feats[select_mask0], p_att_feats[select_mask0], pp_att_feats[select_mask0], p_att_masks, (state[0][:,select_mask0,:],state[1][:,select_mask0,:]))
                    output[select_mask0,:] = output0
                    #Just filling
                    output_adjust[select_mask0,:] = logprob_interven0
                    if self.opt.sup_nde:
                        output_nde[select_mask0,:] = logprob_nde
                    tmp_state[0][:,select_mask0,:] = state0[0]
                    tmp_state[1][:,select_mask0,:] = state0[1]
                if (adjust_type[:,i] == 1).sum() !=0:
                    select_mask1 = (adjust_type[:,i] == 1)
                    if self.opt.sup_nde:
                        logprobs, state1, logprob_interven1, logprob_nde = self.get_logprobs_state(it[select_mask1], p_fc_feats[select_mask1], p_att_feats[select_mask1], pp_att_feats[select_mask1], p_att_masks, (state[0][:,select_mask1,:],state[1][:,select_mask1,:]), 1)
                    else:
                        logprobs, state1, logprob_interven1 = self.get_logprobs_state(it[select_mask1], p_fc_feats[select_mask1], p_att_feats[select_mask1], pp_att_feats[select_mask1], p_att_masks, (state[0][:,select_mask1,:],state[1][:,select_mask1,:]), 1)
                    output[select_mask1,:] = logprobs
                    output_adjust[select_mask1,:] = logprob_interven1
                    if self.opt.sup_nde:
                        output_nde[select_mask1,:] = logprob_nde
                    tmp_state[0][:,select_mask1,:] = state1[0]
                    tmp_state[1][:,select_mask1,:] = state1[1]
                if (adjust_type[:,i] == 2).sum() !=0:
                    select_mask2 = (adjust_type[:,i] == 2)
                    if self.opt.sup_nde:
                        logprobs, state2, logprob_interven2, logprob_nde = self.get_logprobs_state(it[select_mask2], p_fc_feats[select_mask2], p_att_feats[select_mask2], pp_att_feats[select_mask2], p_att_masks, (state[0][:,select_mask2,:],state[1][:,select_mask2,:]), 2)
                    else:
                        logprobs, state2, logprob_interven2 = self.get_logprobs_state(it[select_mask2], p_fc_feats[select_mask2], p_att_feats[select_mask2], pp_att_feats[select_mask2], p_att_masks, (state[0][:,select_mask2,:],state[1][:,select_mask2,:]), 2)
                    output[select_mask2,:] = logprobs
                    output_adjust[select_mask2,:] = logprob_interven2
                    if self.opt.sup_nde:
                        output_nde[select_mask2,:] = logprob_nde
                    tmp_state[0][:,select_mask2,:] = state2[0]
                    tmp_state[1][:,select_mask2,:] = state2[1]
                state = tmp_state
            else:
                if self.opt.sup_nde:
                    output, state, _, _ = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
                else:
                    output, state, _ = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            outputs[:, i] = output
            if self.opt.cexe and adjust_type != None:
                outputs_adjust[:, i] = output_adjust
                if self.opt.sup_nde:
                    outputs_nde[:, i] = output_nde


        if self.opt.cexe:
            if self.opt.sup_nde:
                return (outputs,outputs_adjust, outputs_nde)
            else:
                return (outputs,outputs_adjust)
        else:
            return (outputs,)

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, adjust_type=0):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state, output2 = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, adjust_type)
        #try subtract logit and subtract prob.
        tie = self.logit(output)-self.logit(output2)
        #It is nde if we wipe out visual (or nie if we wipe out language part.)
        #Since we only consider wiping out visual at now, it is nde.
        #todo: Extend.
        nde = self.logit(output2)
        te = self.logit(output)
        logprob_interven = F.log_softmax(tie,dim=1)
        logprobs = F.log_softmax(te, dim=1)
        if self.opt.sup_nde:
            logprob_nde = F.log_softmax(nde,dim=1)

        if self.opt.sup_nde:
            return logprobs, state, logprob_interven, logprob_nde
        else:
            return logprobs, state, logprob_interven
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                if self.opt.sup_nde:
                    logprobs, state, _, _ = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state)
                else:
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, adjust_type= [], opt={}):
        # pdb.set_trace()
        ...
        if self.opt.cec:
            outputs = fc_feats.new_zeros(batch_size, self.seq_length, self.vocab_size+1)
            outputs_tie = fc_feats.new_zeros(batch_size, self.seq_length, self.vocab_size+1)
        ...
        for t in range(self.seq_length + 1):
            ...
            if t < len(adjust_type) :
                if adjust_type[t]:
                    logprobs, state, logprobs_interven = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, adjust_type[t])
                else:
                    logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, adjust_type[t])

            else:
                if self.opt.sup_nde:
                    logprobs, state, logprobs_tie, _ = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
                else:
                    logprobs, state, logprobs_tie = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            ...
            if sample_max:
                ...
                it = it.view(-1).long()
                if t < len(adjust_type) and adjust_type[t] :
                    # logprobs_interven.masked_fill_(self.interven_mask,-float('inf'))
                    sampleLogprobs_interven, it_interven = torch.max(logprobs_interven.data, 1)
                    it_interven = it_interven.view(-1).long()
            ...
            if t < len(adjust_type) and adjust_type[t]:
                seq[:,t] = it_interven
                seqLogprobs[:,t] = sampleLogprobs_interven.view(-1)
            if self.opt.cec:
                outputs[:,t,:] = logprobs
                outputs_tie[:,t,:] = logprobs_tie
            # quit loop if all sequences have finished
            if unfinished.sum() == 0:
                break

        if self.opt.cec:
            return seq, seqLogprobs, outputs, outputs_tie
        else:
...
class TopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None, adjust_type=0):
        prev_h = state[0][-1]
        ...
        if adjust_type == 1:
            # set to mean value of traning set or zero vector.
            # lang_lstm_input2 = torch.cat([self.att_interven, h_att], 1)
            lang_lstm_input2 = torch.cat([torch.zeros_like(att), h_att], 1)
            h_lang2, c_lang2 = self.lang_lstm(lang_lstm_input2, (state[0][1], state[1][1]))
            output2 = F.dropout(h_lang2, self.drop_prob_lm, self.training)
        elif adjust_type ==2 :
            h_att_interven = torch.zeros_like(h_att)
            lang_lstm_input2 = torch.cat([att, h_att_interven], 1)
            h_lang2, c_lang2 = self.lang_lstm(lang_lstm_input2, (state[0][1], state[1][1]))
            output2 = F.dropout(h_lang2, self.drop_prob_lm, self.training)
        else:
            #Prepare for the calculation of NDE
            lang_lstm_input2 = torch.cat([torch.zeros_like(att), h_att], 1)
            h_lang2, c_lang2 = self.lang_lstm(lang_lstm_input2, (state[0][1], state[1][1]))
            output2 = F.dropout(h_lang2, self.drop_prob_lm, self.training)
