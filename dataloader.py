...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.dataset == 'robust-coco':
            robust_split = json.load(open('data/split_robust_coco.json','r'))
            robust_train_id =  [item['img_id'] for item in robust_split['train_id']]
            robust_val_id =  [item['img_id'] for item in robust_split['val_id']]
            robust_test_id =  [item['img_id'] for item in robust_split['test_id']]
            for ix in range(len(self.info['images'])):
                img = self.info['images'][ix]
                if img['id'] in robust_val_id:
                    self.split_ix['val'].append(ix)
                elif img['id'] in robust_test_id:
    ...
    def get_captions(self, ix, seq_per_img):
        ...
        if ncap < seq_per_img:
            ...
            sents_mask = np.zeros([seq_per_img, self.seq_length], dtype = 'int')
            for q in range(seq_per_img):
                ixl = random.randint(ix1,ix2)
                seq[q, :] = self.h5_label_file['labels'][ixl, :self.seq_length]
                sents_mask[q, :] = self.h5_label_file['sents_mask'][ixl, :self.seq_length]
        else:
            ixl = random.randint(ix1, ix2 - seq_per_img + 1)
            seq = self.h5_label_file['labels'][ixl: ixl + seq_per_img, :self.seq_length]
            sents_mask = self.h5_label_file['sents_mask'][ixl: ixl + seq_per_img, :self.seq_length]

        return seq, sents_mask
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            tmp_sents_mask = np.zeros([seq_per_img, self.seq_length + 2], dtype = 'int')
            # tmp_label[:, 1 : self.seq_length + 1] = self.get_captions(ix, seq_per_img)
            tmp_label[:, 1 : self.seq_length + 1], tmp_sents_mask[:, 1 : self.seq_length + 1] = self.get_captions(ix, seq_per_img)
            label_batch.append(tmp_label)
            sents_mask_batch.append(tmp_sents_mask)
        ...
        fc_batch, att_batch, box_batch, label_batch, gts, infos,sents_mask_batch  = \
            zip(*sorted(zip(fc_batch, att_batch, box_batch, label_batch, gts, infos, sents_mask_batch), key=lambda x: 0, reverse=True))
        data = {}
        ...
        data['sents_mask'] = np.vstack(sents_mask_batch)
        # generate mask
