...
def language_eval(dataset, preds, model_id, split, eval_chair):
    import sys
    ...
    elif  'robust-coco' == dataset:
        annFile = 'coco-caption/annotations/captions_robust_val_test.json'
    else:
        raise
...
def eval_split(model, crit, loader, eval_kwargs={}):
    eval_chair = eval_kwargs.get('eval_chair',0)
    ...
    if lang_eval == 1:
        if eval_chair:
            lang_stats, outfile_path = language_eval(dataset, predictions, eval_kwargs['id'], split, eval_chair)
        else:
            lang_stats = language_eval(dataset, predictions, eval_kwargs['id'], split, eval_chair)

    
    # Switch back to training mode
    model.train()
    if eval_chair:
        return loss_sum/loss_evals, predictions, lang_stats, outfile_path
    else:
